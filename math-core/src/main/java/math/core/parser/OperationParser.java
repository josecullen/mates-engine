...
//				operation.getOperation().setSign(hasSign);
	...
	/**
	 * Detecta si la oparacin padre de la expresin tiene signo.
	 * Ejemplo
	 * <ul>
	 * 	<li><p>-(p or q) or p </p><p>No tiene signo, ya que la operacin padre es el ltimo 'or'</p></li>
	...
	private boolean hasSign(String expression){
		...
		if(expression.startsWith("-")){
			...
			/*
			 * Luego de quitar el '-' Quedara
			 * ( ... ) or (...) si el '-' no corresponde a la operacin padre y
			 * ((...) or ( ...)) si el '-' corresponde a la operacin padre.
			 *
			 *  Si llamo a stripOuterBraces y me quita los parntesis, quiere decir que el '-' que quit era de la operacin padre.
			 */
	...
	/**
	 * Quita los parntesis que estn de ms.
	 * Si tiene signo la operacin, tambin quita los parntesis y el signo.
	 * @param expression
	 * @return
	 */
