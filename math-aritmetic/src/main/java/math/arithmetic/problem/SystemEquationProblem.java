...
import static math.arithmetic.operand.ArithmeticVariableUtil.getValuesWithDivisionFactor;
...
public class SystemEquationProblem implements Problem {
	...
	public SystemEquationProblem(EquationLevel level, ArithmeticVariableBuilder avbConst,
			ArithmeticVariableBuilder avbX, OperationBuilder operationBuilder) {
		this.level = level;
	...
	public String getLevel1Expression(ListOperationEjecutor listOperation) {
		...
		listOperation.left(a.getValue() * x.getValue()).and(operations.get(0), b.getValue());

		problemExpression = getValueString(a) + "x " + operations.get(0).getLaTex() + " " + getValueString(b) + " = "
				+ getValueString(listOperation.resolve());
	...
	public String getLevel2Expression(ListOperationEjecutor listOperation) {
		...
		listOperation.left(a.getValue() * x.getValue()).and(operations.get(0), b.getValue() * y.getValue())
				.and(operations.get(1), c.getValue());

		problemExpression = getValueString(a) + "x " + operations.get(0).getLaTex() + " " + getValueString(b) + "y "
				+ operations.get(1).getLaTex() + " " + getValueString(c) + " = "
				+ getValueString(listOperation.resolve());
	...
	public String getLevel3Expression(ListOperationEjecutor listOperation) {
		...
		listOperation.left(a.getValue() * x.getValue()).and(operations.get(0), b.getValue() * y.getValue())
				.and(operations.get(1), c.getValue() * z.getValue()).and(operations.get(2), d.getValue());

		problemExpression = getValueString(a) + "x " + operations.get(0).getLaTex() + " " + getValueString(b) + "y "
				+ operations.get(1).getLaTex() + " " + getValueString(c) + "z " + operations.get(2).getLaTex()
	...
	public String[] getAnswer() {
		...
		case LEVEL_2:
			return new String[] { "x = " + getValueString(x.getValue()) + "\\: y = " + getValueString(y.getValue()) };
		case LEVEL_3:
			return new String[] { "x = " + getValueString(x.getValue()) + "\\: y = " + getValueString(y.getValue())
	...
	public String[] getAnswerOptions(int options) {
		...
		posibleAnswers = getValuesWithDivisionFactor(20, x.getValue(), avbX.getDivisionFactor());
		List<String> posiblesAsString = posibleAnswers.stream().map(posibleAnswer -> {
			return getValueString(posibleAnswer);
		...
		case LEVEL_1:
			...
			break;
		case LEVEL_2:
			result[1] = "x = " + posiblesAsString.remove((int) Math.random() * posiblesAsString.size()) + "\\: y = "
					+ posiblesAsString.remove((int) Math.random() * posiblesAsString.size());
			result[2] = "x = " + posiblesAsString.remove((int) Math.random() * posiblesAsString.size()) + "\\: y = "
					+ posiblesAsString.remove((int) Math.random() * posiblesAsString.size());
			break;
		case LEVEL_3:
			result[1] = "x = " + posiblesAsString.remove((int) Math.random() * posiblesAsString.size()) + "\\: y = "
					+ posiblesAsString.remove((int) Math.random() * posiblesAsString.size()) + "\\: z = "
					+ posiblesAsString.remove((int) Math.random() * posiblesAsString.size());
			result[2] = "x = " + posiblesAsString.remove((int) Math.random() * posiblesAsString.size()) + "\\: y = "
					+ posiblesAsString.remove((int) Math.random() * posiblesAsString.size()) + "\\: z = "
					+ posiblesAsString.remove((int) Math.random() * posiblesAsString.size());
