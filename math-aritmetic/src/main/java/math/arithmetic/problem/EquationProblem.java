...
public class EquationProblem implements Problem {
	 ...
	 */
	ArithmeticVariable a, x1, x2, x3;

	String problemExpression, solvedExpression, answer;
	String[] answerOptions;

	EquationLevel level;

	public EquationProblem(EquationLevel level, ArithmeticVariable a, ArithmeticVariable x1, ArithmeticVariable x2,
			ArithmeticVariable x3) {
		this.level = level;
		this.a = a;
		this.x1 = x1;
		this.x2 = x2;
	...
	public String[] getAnswerOptions(int options) {
		...
		try {
			...
			for (int i = correctsCount; i < answers.length; i++) {
				answers[i] = ArithmeticVariableUtil.getNoRepeatedAnswer(answers[(int) (Math.random() * correctsCount)],
						answers, 1);
	...
	private String getLevel1Expression() {
		...
		expression.append(ArithmeticVariableUtil.getValueString(a)).append("x").append(" + ")
					.append(ArithmeticVariableUtil.getValueString(b)).append(" = 0");
	...
	private String getLevel2Expression() {
		...
		double b = -1 * a.getValue() * (x1.getValue() + x2.getValue()); // -a(x1
																		// + x2)
		double c = a.getValue() * x1.getValue() * x2.getValue();

		expression.append(ArithmeticVariableUtil.getValueString(a)).append("x^2").append(" + ")
			.append(ArithmeticVariableUtil.getValueString(b)).append("x").append(" + ")
			.append(ArithmeticVariableUtil.getValueString(c)).append(" = 0");
	...
	private String getLevel3Expression() {
		...
		double b = -1 * a.getValue() * (x1.getValue() + x2.getValue() + x3.getValue()); // -a(
																						// x1
																						// +
																						// x2
																						// +
																						...
																						// )
		double c = a.getValue()
				* (x1.getValue() * x2.getValue() + x1.getValue() * x3.getValue() + x2.getValue() * x3.getValue()); // -a(
																													// x1
																													// *
																													// x2
																													// +
		...
		expression.append(ArithmeticVariableUtil.getValueString(a)).append("x^3").append(" + ")
			.append(ArithmeticVariableUtil.getValueString(b)).append("x^2").append(" + ")
			.append(ArithmeticVariableUtil.getValueString(c)).append("x").append(" + ")
			.append(ArithmeticVariableUtil.getValueString(d)).append(" = 0");
